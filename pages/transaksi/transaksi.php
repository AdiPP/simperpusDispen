...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <a href="tambahpeminjaman.php" class="btn btn-default margin-bottom" style="margin-right: 20px;"><i class="fa fa-upload"></i> Tambah Peminjaman</a>
          <a href="tambahpengembalian.php" class="btn btn-default margin-bottom" style="margin-right: 20px;"><i class="fa fa-download"></i> Tambah Pengembalian</a>
          <a href="reportdatatransaksi.php" class="btn btn-default margin-bottom pull-right"><i class="glyphicon glyphicon-print"></i> Laporan Transaksi</a>
        </div>
        <!-- /.col -->
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <th>Nama Anggota</th>
                  <th>Judul Pustaka</th>
                  <!-- <th style="text-align: center">Batas Pengembalian</th> -->
                  <th>Aksi</th>
                </tr>
                </thead>
                <tbody>
                  <?php
                  $nomor = 1;
                  $peminjaman = mysqli_query($koneksi, "SELECT * FROM t_peminjaman ORDER BY id_pmnjmn DESC;");
                  while ($p = mysqli_fetch_array($peminjaman)) {
                    $id_anggota = $p['id_anggota'];
                    $anggota = mysqli_query($koneksi, "SELECT nama_anggota FROM t_dataanggota WHERE id_anggota = $id_anggota");
                    $a = mysqli_fetch_assoc($anggota);
                    $id_pustaka = $p['id_dp'];
                    $pustaka = mysqli_query($koneksi, "SELECT judul_dp FROM t_datapustaka WHERE id_dp = $id_pustaka");
                    $pu = mysqli_fetch_assoc($pustaka);
                    $id_pengguna = $p['id_pengguna'];
                    $pengguna = mysqli_query($koneksi, "SELECT nama_pengguna FROM t_datapengguna WHERE id_pengguna = $id_pengguna");
                    $peng = mysqli_fetch_assoc($pengguna);
                   ?>
                <tr>
                  <td align="center"><?php echo $nomor; ?>.</td>
                  <td><?php
                  $newDate = date("d/m/y", strtotime($p['tgl_pjm']));
                  echo $newDate;
                  ?></td>
                  <td><?php echo $p['nomor_pmnjmn']; ?></td>
                  <td><?php echo $a['nama_anggota']; ?></td>
                  <td><?php echo $pu['judul_dp']; ?></td>
                  <!-- <td><?php
                  $newDate = date("d/m/Y", strtotime($p['tgl_kmbl']));
                  echo $newDate;
                  ?></td> -->
                  <td style="text-align: center;">
                    <a href="informasipeminjaman.php?id=<?php echo $p['id_pmnjmn'] ?>" class="btn btn-info btn-xs btn-flat"><i class="fa fa-eye"></i></a>
                  </td>
                </tr>
                  <?php
                  $nomor++;
            ...
            <div class="box-body">
              <table id="example2" class="table table-bordered table-striped">
                ...
                <tbody>
                  <?php
                    $nomor = 1;
                    $pengembalian = mysqli_query($koneksi, "SELECT * FROM t_pengembalian ORDER BY id_pgmbln DESC");
                    while ($p = mysqli_fetch_array($pengembalian)){
                      $id_pmnjmn = $p['id_pmnjmn'];
                      $peminjaman = mysqli_query($koneksi, "SELECT tgl_pjm, tgl_kmbl, nomor_pmnjmn FROM t_peminjaman WHERE id_pmnjmn = $id_pmnjmn");
                ...
                <tr>
                  ...
                  echo $newDate;
                  ?></td>
                  <td><?php echo $p['nomor_pgmbln']; ?></td>
                  <!-- <td style="text-align: center">#<?php echo $pe['nomor_pmnjmn']; ?></td> -->
                  <td><?php echo $a['nama_anggota']; ?></td>
                  <td><?php echo $pu['judul_dp']; ?></td>
                  <!-- <td><?php
                  $newDate = date("d/m/Y", strtotime($pe['tgl_pjm']));
                  echo $newDate;
                  ?></td> -->
                  <td><?php
                  if ($p['ktrlmbtn'] != 0) {
                  ...
                  ?></td>
                  <td style="text-align: center;">
                    <a href="informasipengembalian.php?id=<?php echo $p['id_pgmbln'] ?>" class="btn btn-info btn-xs btn-flat"><i class="fa fa-eye"></i></a>
                  </td>
                </tr>
                <?php
                  $nomor++;
