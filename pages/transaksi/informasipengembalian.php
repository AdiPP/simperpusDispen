...
<head>
  ...
  <title>SIMPERPUS | Informasi Pengembalian</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    <?php
      $pengembalian = mysqli_query($koneksi,"SELECT * FROM t_pengembalian WHERE id_pgmbln = '$id'");
      while($pgmbln = mysqli_fetch_array($pengembalian)){
        $idpustaka = $pgmbln['id_dp'];
        $datapustaka = mysqli_query($koneksi, "SELECT * FROM t_datapustaka WHERE id_dp = '$idpustaka'");
        $dp = mysqli_fetch_assoc($datapustaka);
        $idanggota = $pgmbln['id_anggota'];
        $dataanggota = mysqli_query($koneksi, "SELECT * FROM t_dataanggota WHERE id_anggota = '$idanggota'");
        $da = mysqli_fetch_assoc($dataanggota);
        $idpeminjaman = $pgmbln['id_pmnjmn'];
        $datapeminjaman = mysqli_query($koneksi, "SELECT * FROM t_peminjaman WHERE id_pmnjmn = '$idpeminjaman'");
        $dpeminjaman = mysqli_fetch_assoc($datapeminjaman);
        $idpengguna = $pgmbln['id_pengguna'];
        $datapengguna = mysqli_query($koneksi, "SELECT * FROM t_datapengguna WHERE id_pengguna = '$idpengguna'");
        $dpengguna = mysqli_fetch_assoc($datapengguna);
     ?>
    <!-- Content Header (Page header) -->
    <section class="content-header">
      <h1>
        Informasi Pengembalian
        <small>#<?php echo $pgmbln['nomor_pgmbln']; ?></small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="../../index.php"><i class="fa fa-home"></i> Beranda</a></li>
        <li><a href="transaksi.php">Transaksi</a></li>
        <li class="active">Informasi Pengembalian</li>
      </ol>
    </section>

    <div class="pad margin no-print">
      <div class="callout callout-info" style="margin-bottom: 0!important;">
        <h4><i class="fa fa-info"></i> Sistem:</h4>
        Data pada halaman ini tidak dapat di ubah.
      </div>
    </div>
    ...
    <section class="invoice">
      ...
      <div class="row">
        <div class="col-xs-12">
          <h2 class="page-header">
            ...
            <small class="pull-right">Nomor Peminjaman: #<?php echo $dpeminjaman['nomor_pmnjmn'] ?></small>
          </h2>
        </div>
        <!-- /.col -->
      </div>
          ...
          <div class="table-responsive">
            <table class="table table-striped">
              <tr>
                ...
                <td>#<?php echo $dpeminjaman['nomor_pmnjmn'] ?></td>
              </tr>
              <tr>
                <th>Nomor Pustaka </th>
              ...
              <tr>
                <th>Judul </th>
                <td><?php echo $dp['judul_dp'] ?></td>
              </tr>
              <tr>
                <th>Nomor Anggota</th>
                <td>#<?php echo $da['no_anggota'] ?></td>
              </tr>
              <tr>
                <th>Nama </th>
              ...
              <tr>
                ...
                <td><?php
                $tanggal = date("d/m/Y", strtotime($dpeminjaman['tgl_pjm']));
                echo $tanggal;
                ?></td>
              </tr>
              <tr>
                ...
                <td><?php
                $tanggal = date("d/m/Y", strtotime($dpeminjaman['tgl_kmbl']));
                echo $tanggal;
                ?></td>
              </tr>
            </table>
      ...
      <div class="row">
        <div class="col-xs-12">
          <h2 class="page-header">
            ...
            <small class="pull-right">Nomor Pengembalian: #<?php echo $pgmbln['nomor_pgmbln'] ?></small>
          </h2>
        </div>
        <!-- /.col -->
      </div>
      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="table-responsive">
            <table class="table table-striped">
              <tr>
                <th style="width:50%">Nomor Pengembalian</th>
                <td>#<?php echo $pgmbln['nomor_pgmbln'] ?></td>
              </tr>
              <tr>
                <th>Admin</th>
                <td><?php echo $dpengguna['nama_pengguna'] ?></td>
              </tr>
              <tr>
                <th>NIP</th>
                <td><?php echo $dpengguna['nip'] ?></td>
              </tr>
              <tr>
                <th>Tanggal Kembali</th>
                <td><?php
                $tanggal = date("d/m/Y", strtotime($pgmbln['tgl_kmbl']));
                echo $tanggal;
                ?></td>
              </tr>
              <tr>
      ...
      <div class="row no-print">
        <div class="col-xs-12">
          <a href="transaksi.php" class="btn btn-default" style="margin-right: 10px;">Kembali</a>
        </div>
      </div>
      <?php
        }
